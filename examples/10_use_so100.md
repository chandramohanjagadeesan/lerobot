...
> [!NOTE Chandra]
> I Prefered setting the ID for one motor assembling the subsequent arm and then disconeeting the power supply and repeating the process for other motors and parts. This is due to the problems that i faced during the first assembly process. When all the IDs are set togehter then the power supply is remived and the horns of the motors and next arm is assembled. When the arm is assembled the horn moves since it does not have power supply so the posittions get changed reseeting the calibration. Instead if one motor ID is set and the arm is added when the motor's power supply is on. The positoin of the shat cannot change due to power making the process faster.

> [!NOTE Chandra]
> If there are any errors in confiuring the motors. Downlaod the software from https://www.feetechrc.com/software.html and change the IDs other values
